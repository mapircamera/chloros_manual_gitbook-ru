...
Chloros доступно в 4 режимах применения:

## Chloros: настольное приложение с графическим интерфейсом

\[Рекомендуется] Автономное отдельное окно со всеми функциями.

## Chloros (браузер): веб-интерфейс с графическим интерфейсом

Работает в веб-браузере (Chrome/Edge). Лучше всего подходит для медленных компьютеров с &lt;4 ГБ видеопамяти.
...
* **Ускорение GPU (CUDA)**: используйте преимущества современных вариантов памяти GPU с большим объемом для дальнейшего ускорения конвейера обработки изображений. Для достижения наилучших результатов мы рекомендуем 4 ГБ или более VRAM.
...
* **Chloros+** [**API**](api-python-sdk.md) **Доступ:** запустите Chloros+ из Python для программного управления, что обеспечит беспрепятственную интеграцию с вашими исследовательскими процессами, рабочими потоками анализа данных и пользовательскими приложениями.
