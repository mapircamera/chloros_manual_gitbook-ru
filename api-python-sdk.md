...
| -------------------- | ------------------------------------------------------------------- |
| **Chloros Desktop**  | Должен быть установлен локально                                           |
...
{% hint style=&quot;info&quot; %}
**Выход из системы**: Пользователи SDK могут программно очистить кэшированные учетные данные с помощью метода `logout()`. См. [метод logout()](#logout) в справочнике API.
{% endhint %}
...
**Возвращает:** `dict` — состояние бэкэнда**Пример:**
...
**Возвращаемые значения:** `dict` — результаты обработки**Пример:**
...
3. Если возникли проблемы с учетными данными, очистите кэш учетных данных и войдите в систему заново:
...
### Таймаут обработки**Проблема:** Таймаут обработки**Решения:**
...
### Порт уже используется**Проблема:** Порт 5000 бэкэнда занят**Решения:**
...
| Функция         | Desktop GUI | CLI Командная строка | Python SDK  |
| --------------- | ----------- | ---------------- | ----------- |
