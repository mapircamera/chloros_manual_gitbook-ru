...
Настройки проекта <img src="../.gitbook/assets/icon_project-settings.JPG" alt="" data-size="line"> в Chloros позволяют настроить все аспекты обработки изображений, обнаружения калибровочных мишеней, расчета мультиспектральных индексов и параметров экспорта для вашего проекта. Эти настройки сохраняются вместе с вашим проектом и могут быть сохранены в качестве шаблонов для повторного использования в нескольких проектах.
...
2. Нажмите вкладку **Настройки проекта**  <img src="../.gitbook/assets/icon_project-settings.JPG" alt="" data-size="line"> на левой боковой панели
...
* **По умолчанию**: 60
* **Описание**: Управляет порогом кластеризации для группировки областей схожих цветов при обнаружении целей калибровки. Более высокие значения требуют группировки более схожих цветов, что приводит к более консервативному обнаружению целей. Более низкие значения допускают большее разнообразие цветов в пределах группы целей.
* **Когда настраивать**:
...
* **Параметры**:
  ...
  * С учетом текстуры (медленный, максимальное качество) \[Chloros+]
* **По умолчанию**: Стандартный (быстрый, среднее качество)
* **Описание**: Выбирает алгоритм демозаикирования, используемый для преобразования необработанных данных датчика с матрицей Байера в полноцветные изображения. Метод «Стандартный (быстрый, среднее качество)» обеспечивает оптимальный баланс между скоростью обработки и качеством изображения. Метод «С учетом текстуры (медленный, максимальное качество)» \[Chloros+] использует высококачественный дебайеринг с учетом краев в сочетании с моделью шумоподавления на основе искусственного интеллекта/машинного обучения, которая удаляет практически весь шум дебайеринга. Для работы модели с учетом текстуры требуется память графического процессора (VRAM). Мы рекомендуем использовать его, если у вас есть более 4 ГБ VRAM для более быстрой обработки.
...
* **Описание**: Устанавливает минимальный интервал времени (в секундах) между использованием калибровочных мишеней. При установке значения 0 Chloros будет использовать каждую обнаруженную калибровочную мишень. При установке более высокого значения Chloros будет использовать только калибровочные мишени, разделенные как минимум таким количеством секунд, что сокращает время обработки наборов данных с частым захватом калибровочных мишеней.
* **Когда настраивать**:
  * Установите значение 0 для максимальной точности калибровки при изменяющихся условиях освещения.
  * Увеличьте значение (например, до 60–300 секунд) для более быстрой обработки при постоянном освещении и частых изображениях калибровочных мишеней.
...
* **Когда настраивать**: Установите значение, соответствующее смещению вашего местного часового пояса, если в данных PPK используется местное время вместо UTC. Например:
  * Тихоокеанское время: -8 или -7 (в зависимости от летнего времени)
  * Восточное время: -5 или -4 (в зависимости от летнего времени)
  * Центральноевропейское время: +1 или +2 (в зависимости от летнего времени)
...
* **Требование**: в каталоге проекта должен присутствовать файл журнала .daq с записями о выдержке.
* **Когда включать**: рекомендуется всегда включать корректировку PPK, если в файле журнала .daq есть записи об обратной связи по выдержке.
...
* **Описание**: Назначает определенную камеру для контакта экспозиции 1 для синхронизации времени PPK. Контакт экспозиции записывает точное время срабатывания затвора камеры, что имеет решающее значение для точной геолокации PPK.
...
* **Тип**: выпадающий список
* **Видимость**: отображается только при включенной опции «Применить поправки PPK» И наличии данных экспозиции для контакта 2
* **Параметры**:
  * Названия моделей камер, обнаруженных в проекте
  * «Не использовать» — игнорировать этот контакт экспозиции
* **По умолчанию**: автоматический выбор на основе конфигурации проекта
* **Описание**: назначает определенную камеру контакту экспозиции 2 для синхронизации времени PPK при использовании конфигурации с двумя камерами.
* **Поведение автоматического выбора**:
  ...
  * Несколько камер: требуется ручной выбор
* **Примечание**: одну и ту же камеру нельзя назначить одновременно для контакта 1 и контакта 2.***
...
* **Тип**: Панель настройки специальных индексов.
* **Описание**: Открывает интерактивную панель, на которой можно выбрать и настроить мультиспектральные индексы растительности (NDVI, NDRE, EVI и т. д.) для расчета во время обработки изображений. Вы можете добавить несколько индексов, каждый со своими настройками визуализации.
...
* NDVI (нормализованный разностный растительный индекс)
  ...
  * И многие другие (полный список см. в [Формулы многоспектральных индексов](multispectral-index-formulas.md))
* **Особенности**:
  * Выбор из предварительно определенных формул индекса
  * Настройка цветовых градиентов визуализации (LUT — таблицы просмотра)
  * Установка пороговых значений для анализа
  * Создание пользовательских формул индекса

### Пользовательские формулы (функция Chloros+)

* **Тип**: массив определений пользовательских формул
* **Описание**: позволяет создавать и сохранять пользовательские формулы мультиспектральных индексов с использованием математических операций с полосами. Пользовательские формулы сохраняются вместе с настройками проекта и могут использоваться так же, как встроенные индексы.
* **Как создать**:
  1. В панели настроек индекса найдите опцию пользовательской формулы.
...
* **Параметры**:
  ...
  * **TIFF (32-разрядный, проценты)** — 32-разрядный формат TIFF с плавающей запятой и значениями отражения в процентах
  * **PNG (8-разрядный)** - Сжатый 8-разрядный формат PNG.
  * **JPG (8-разрядный)** - Сжатый 8-разрядный формат JPEG.
* **По умолчанию**: TIFF (16-разрядный)
...
* **Рекомендации по формату**:
  ...
  * **TIFF (32-разрядный, проценты)**: Лучше всего подходит для рабочих процессов, требующих значений отражения в процентах (0-100%). Обеспечивает максимальную точность радиометрических измерений.
  * **PNG (8-разрядный)**: Хорошо подходит для просмотра в Интернете и общей визуализации. Меньший размер файлов с сжатием без потерь, но уменьшенный динамический диапазон.
  * **JPG (8-битный)**: Наименьший размер файлов, лучше всего подходит только для предварительного просмотра и отображения в Интернете. Использует сжатие с потерями, которое не подходит для научного анализа.***
...
* **Описание**: введите описательное имя для шаблона настроек и нажмите значок «Сохранить». Шаблон сохранит все текущие настройки проекта (обнаружение цели, параметры обработки, индексы и формат экспорта) для удобного повторного использования в будущих проектах.
