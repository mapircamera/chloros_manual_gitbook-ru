...
* **Когда настраивать**:
  * Увеличьте, если вы получаете ложные срабатывания на небольших артефактах изображения.
  * Уменьшите, если ваши калибровочные цели выглядят небольшими на изображениях и не обнаруживаются.
...
* **Тип**: флажок
* **По умолчанию**: включено (отмечено)
* **Описание**: Включает автоматическую калибровку отражательной способности с использованием обнаруженных калибровочных целей в ваших изображениях. Это нормализует значения отражательной способности во всем наборе данных и обеспечивает согласованные измерения независимо от условий освещения.
...
* **Когда настраивать**: Установите это значение в соответствии со смещением вашего местного часового пояса, если ваши данные PPK используют местное время вместо UTC. Например:
...
* **Описание**: Включает использование поправок Post-Processed Kinematic (PPK) из регистраторов MAPIR DAQ, содержащих GPS (GNSS). При включении Chloros будет использовать любые файлы журнала .daq, содержащие данные о контактах экспозиции в каталоге вашего проекта, и применять точные корректировки геолокации к вашим изображениям.
* **Требование**: файл журнала .daq с записями о контактах экспозиции должен присутствовать в каталоге вашего проекта
* **Когда включать**: рекомендуется всегда включать корректировку PPK, если в файле журнала .daq есть записи об обратной связи по экспозиции.

### Контакт экспозиции 1

* **Тип**: Выпадающий список
* **Видимость**: Виден только при включении «Применить корректировки PPK» И наличии данных экспозиции для контакта 1.
* **Опции**:
...
* **Доступные индексы**: Система включает более 30 предварительно определенных мультиспектральных индексов, в том числе:
  ...
  * И многое другое (полный список см. в [Формулы мультиспектральных индексов](multispectral-index-formulas.md))
* **Особенности**:
...
* **Как создать**:
  1. На панели настройки индекса найдите опцию пользовательской формулы.
...
* **Тип**: выпадающий список
* **Опции**:
  * **TIFF (16-битный)** - Несжатый 16-разрядный формат TIFF
