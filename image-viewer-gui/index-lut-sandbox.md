...
* **Цель**: облегчить восприятие и интерпретацию паттернов**Таблица перевода в оттенки серого и таблица перевода в цвета:**

* Оттенки серого: научные и нейтральные, отображают исходные данные
* Таблица перевода в цвета: интуитивная и эффектная, выделяет паттерны и различия
...
{% hint style=&quot;success&quot; %}
**Рекомендация**: для индексов растительности, таких как NDVI, градиент Red-Yellow-Green является наиболее интуитивным, поскольку он соответствует естественным цветовым ассоциациям (зеленый = здоровый, желтый = умеренный, красный = стресс).
{% endhint %}

### Настройка цветовых классов

**Элемент управления «Классы»**определяет, сколько дискретных цветовых ступеней будет отображаться в градиенте:**Варианты количества классов:*** **2–5 классов**: очень широкие категории, четкие зоны
* **6-10 классов**: сбалансированные, подходят для классификации
* **11-20 классов**: плавные градиенты, непрерывный вид
* **20+ классов**: почти непрерывные, максимальная плавность**Как настроить:**
...
* **Больше классов** (15-20): Плавные переходы, детальные вариации, фотографический вид**Когда использовать:*** **Немного классов (3-5)**: Презентационные слайды, карты классификации, простые отчеты
...
**Элементы управления диапазоном значений**определяют, какие значения индекса сопоставляются с какими цветами в вашем градиенте:**Элементы управления диапазоном на панели LUT:*** **Минимальное значение**: нижняя граница цветовой шкалы
...
6. Визуализация **обновится немедленно**{% hint style=&quot;info&quot; %}**Автоматическое масштабирование**: при первом применении LUT Chloros автоматически устанавливает минимальное/максимальное значение в соответствии с фактическим диапазоном данных в изображении. Затем вы можете сузить этот диапазон, чтобы сосредоточиться на конкретных диапазонах значений, представляющих интерес.
{% endhint %}

**Примеры настройки диапазона NDVI:*** **Полный диапазон**: `-1.0` до `1.0` (показать все возможные значения)
* **С фокусом на растительность**: `0.2` до `0.9` (исключить голую почву и воду)
* **Только здоровая растительность**: `0.5` до `0.9` (выделять только сильные растения)
* **Обнаружение стресса**: `0.2` до `0.5` (выделять проблемные области)
* **Пользовательский диапазон**: настраивать на основе наблюдаемых значений пикселей**Зачем настраивать диапазоны?*** **Увеличить контраст** в интересующей вас области
* **Исключение нерелевантных значений** (например, водоемы, голая почва)
...
{% hint style=&quot;warning&quot; %}
**Ограничение прозрачности**: прозрачные пиксели будут отображаться в виде фонового цвета в просмотрщике. При экспорте во время обработки прозрачность сохраняется в формате PNG, но не в формате JPG.
{% endhint %}

#### 3. Фон индекса

* Пиксели **за пределами диапазона**отображаются в**оттенках серого** (показывая исходные значения индекса)
* Пиксели **в пределах диапазона**отображаются**цветовым градиентом*** **Пример использования**: тонкое выделение, сохранение контекста при одновременном выделении областей, представляющих интерес
* **Пример**: выделение цветом стрессовой растительности (NDVI 0,3-0,5) с отображением здоровых областей в сером цвете

#### 4. Оригинальный фон

* Пиксели **за пределами диапазона**отображаются в виде**оригинального мультиспектрального изображения*** Пиксели **в пределах диапазона**отображаются в виде**цветового градиента*** **Пример использования**: Наиболее интуитивный — сочетает естественный контекст изображения с аналитической цветовой накладкой
* **Пример**: Просмотр фактического вида поля/посевов с наложенными областями стресса, обозначенными цветом
...
| **Фон индекса**       | Незначительное выделение, сохранение контекста данных  | Цвет в диапазоне, серый за пределами  |
| **Оригинальный фон**    | Отчеты, презентации, интуитивный анализ | Цвет в диапазоне, фотография за пределами |
...
7. Градиент **немедленно обновится** на изображении.**Добавление или удаление цветовых остановок:*** **Добавить остановку**: щелкните значок +, чтобы добавить новый образец в конец.
* **Удалить остановку**: дважды щелкните квадрат цвета, чтобы удалить образец.**Стратегии настройки:*** **Инвертировать градиент**: переверните порядок цветов, чтобы изменить значение (например, зеленый = низкий, красный = высокий)
...
* **Многопороговое**: используйте разные цвета для определенных пороговых значений для классификации.

{% hint style=&quot;info&quot; %}
**Сохранение настраиваемых градиентов**: настраиваемые градиенты можно сохранять и использовать повторно. Нажмите значок сохранения на панели LUT, чтобы сохранить настраиваемые цветовые схемы для будущего использования.
{% endhint %}
...
* **Настройка диапазона значений** → Контраст меняется в реальном времени
* **Изменить классы** → Гладкость градиента обновляется немедленно
* **Изменить обрезку** → Отображение фона меняется мгновенно
* **Редактировать цвета** → Пользовательский градиент применяется немедленно**Кнопка «Применить» не нужна** — все изменения происходят в режиме реального времени и интерактивно!
...
3. **Примените градиент LUT** — начните с предустановки Red-Yellow-Green.
...
6. **Выберите обрезку** — попробуйте «Original Background» (Исходный фон) для контекста
...
1. Значения пикселей отображаются, когда для изображения**отмечены** поля «Индекс» или «Индекс и LUT».
...
5. **Запишите** диапазоны значений для различных характеристик:
   ...
   * **Поврежденная растительность**: например, NDVI 0,30-0,50
   * **Голая почва**: например, NDVI 0,05-0,25
   * **Вода** (если присутствует): например, NDVI от -0,05 до 0,10**Использование значений пикселей для настройки диапазонов LUT:**После проверки значений пикселей настройте минимальное/максимальное значение LUT соответствующим образом:**Пример сценария:*** **Наблюдение**: значения почвы = 0,05–0,25, стресс = 0,25–0,50, здоровье = 0,50–0,85
...
{% hint style=&quot;info&quot; %}
**Динамический диапазон**: разные культуры, сезоны и стадии роста будут иметь разные диапазоны значений. Всегда проверяйте значения пикселей в вашем конкретном наборе данных перед установкой диапазонов LUT.
{% endhint %}
...
{% hint style=&quot;info&quot; %}
**Где создавать**: Пользовательские индексы можно настроить в**Настройках проекта** перед обработкой, а также в боковой панели песочницы Image Viewer.
{% endhint %}
...
4. **Определите формулу**, используя переменные диапазона:
