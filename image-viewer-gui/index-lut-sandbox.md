...
{% hint style=&quot;success&quot; %}
**Возможности визуализации**: Применение цветной LUT к индексной изображению в оттенках серого значительно упрощает идентификацию паттернов, аномалий и областей, представляющих интерес, с первого взгляда.
{% endhint %}
...
* **Небольшое количество классов (3-5)**: презентационные слайды, карты классификации, простые отчеты
* **Среднее количество классов (6-10)**: общий анализ, сбалансированная детализация, стандартные отчеты
* **Много классов (15-20)**: научный анализ, подробная проверка, результаты публикационного качества
...
* **Только здоровая растительность**: `0.5` до `0.9` (выделить только сильные растения)
* **Обнаружение стресса**: `0.2` до `0.5` (выделить проблемные области)
* **Пользовательский диапазон**: настройте на основе наблюдаемых значений пикселей

**Зачем настраивать диапазоны?**

* **Увеличьте контраст** в интересующей вас области
* **Исключите нерелевантные значения** (например, водоемы, голая почва)
* **Стандартизируйте визуализацию** для нескольких изображений или дат
* **Выделите незначительные различия** в узком диапазоне значений
...
* Пиксели **выше максимального значения** → отображаются с использованием **последнего цвета** в градиенте (например, зеленый)
...
* **Пример**: выделение цветом стрессовой растительности (NDVI 0,3-0,5) при отображении здоровых областей серым цветом
...
* **Пример использования**: наиболее интуитивный — сочетает естественный контекст изображения с аналитической цветовой накладкой
...
| **Прозрачный фон** | Наложения GIS, выделение определенных диапазонов    | Цвет в диапазоне, за его пределами — пустота |
| **Фон индекса**       | Незначительное выделение, сохранение контекста данных  | Цвет в диапазоне, за его пределами — серый цвет  |
| **Оригинальный фон**    | Отчеты, презентации, интуитивный анализ | Цвет в диапазоне, за его пределами — фотография |
...
{% hint style=&quot;info&quot; %}
**Сохранение настраиваемых градиентов**: настраиваемые градиенты можно сохранять и использовать повторно. Щелкните значок сохранения на панели LUT, чтобы сохранить настраиваемые цветовые схемы для будущего использования.
{% endhint %}
...
2. **Применить индекс** — выберите фильтр камеры и формулу индекса, перетащите цветные кружки в соответствующее место в формуле индекса
...
7. **Уточните цвета** — при необходимости настройте градиент для особого выделения
...
**Использование значений пикселей для установки диапазонов LUT:**
...
3. Найдите опцию **«Пользовательский»** (необходимо войти с лицензией Chloros+).
