...
* **Расчет индекса в режиме реального времени** — мгновенное применение любого индекса растительности
...
3. Нажмите на **выпадающий список слоев** (в правом верхнем углу просмотрщика)
...
4. Перетащите кружки цвета канала фильтра в места в формуле индекса ниже.
...
**Градации серого и цветные LUT:**

* Градации серого: научные и нейтральные, отображают исходные данные
* Цветные LUT: интуитивные и эффектные, выделяют паттерны и различия
...
{% hint style=&quot;success&quot; %}
**Рекомендации**: для индексов растительности, таких как NDVI, градиент Red-Yellow-Green является наиболее интуитивным, поскольку он соответствует естественным цветовым ассоциациям (зеленый = здоровый, желтый = умеренный, красный = стрессовый).
{% endhint %}
...
* **Большее количество классов** (15–20): плавные переходы, детальные вариации, фотографический вид.
...
**Элементы управления диапазоном значений** определяют, какие значения индекса сопоставляются с какими цветами в градиенте:
...
* **Полный диапазон**: от `-1.0` до `1.0` (отображение всех возможных значений)
* **С фокусом на растительность**: от `0.2` до `0.9` (исключить голую почву и воду)
...
* **Пользовательский диапазон**: настраивать на основе наблюдаемых значений пикселей
...
{% hint style=&quot;warning&quot; %}
**Ограничение прозрачности**: прозрачные пиксели будут отображаться в виде цвета фона в просмотрщике. При экспорте во время обработки прозрачность сохраняется в формате PNG, но не в формате JPG.
{% endhint %}
...
* **Пример использования**: тонкое выделение, сохранение контекста при одновременном выделении областей, представляющих интерес
* **Пример**: выделение цветом стрессовой растительности (NDVI 0,3–0,5) при отображении здоровых областей в сером цвете

#### 4. Исходный фон

* Пиксели **за пределами диапазона** отображают **исходное мультиспектральное изображение**
...
* **Пример**: просмотр фактического вида поля/урожая с наложением цветовой кодировки областей стресса

### Выбор правильного режима обрезки
...
| **Фон индекса**       | Незначительное выделение, сохранение контекста данных  | Цвет в диапазоне, за пределами диапазона — серое пространство  |
...
* **Добавить остановку**: щелкните значок +, чтобы добавить новый образец в конце
* **Удаление остановки**: дважды щелкните квадрат цвета, чтобы удалить образец
...
* **Мультипороговое значение**: используйте разные цвета для определенных пороговых значений для классификации

{% hint style=&quot;info&quot; %}
**Сохранение пользовательских градиентов**: пользовательские градиенты можно сохранять и использовать повторно. Нажмите значок сохранения на панели LUT, чтобы сохранить пользовательские цветовые схемы для будущего использования.
{% endhint %}
...
* **Редактировать цвета** → Пользовательский градиент применяется немедленно

**Кнопка «Применить» не нужна** — все изменения происходят в режиме реального времени и интерактивно!

{% hint style=&quot;success&quot; %}
**Мгновенная обратная связь**: мгновенная визуальная обратная связь позволяет быстро экспериментировать с различными настройками, пока вы не найдете оптимальную визуализацию для своих аналитических потребностей.
{% endhint %}

### Итеративный рабочий процесс уточнения
...
5. **Отрегулируйте минимальное/максимальное значение** — сузьте диапазон, чтобы сосредоточиться на растительности (например, от 0,2 до 0,9)
...
1. Значения пикселей отображаются, когда для изображения **установлены флажки** «Индекс» или «Индекс и LUT».
...
5. **Запишите** диапазоны значений для различных характеристик:
   ...
   * **Стрессированная растительность**: например, NDVI 0,30-0,50
   * **Голая почва**: например, NDVI 0,05-0,25
   * **Вода** (при наличии): например, NDVI от -0,05 до 0,10

**Использование значений пикселей для настройки диапазонов LUT:**

После проверки значений пикселей настройте минимальное/максимальное значение LUT соответствующим образом:
...
{% hint style=&quot;info&quot; %}
**Динамический диапазон**: разные культуры, сезоны и стадии роста будут иметь разные диапазоны значений. Всегда проверяйте значения пикселей в вашем конкретном наборе данных, прежде чем устанавливать диапазоны LUT.
{% endhint %}
...
Теперь, когда вы понимаете, как работает песочница индексов/LUT:

* **Применить к обработке**: используйте обнаруженные настройки в [Настройках проекта](../project-settings/project-settings.md)
* **Пакетная обработка**: примените оптимизированные индексы к полным наборам данных
* **Узнать больше**: прочитайте [Формулы мультиспектральных индексов](../project-settings/multispectral-index-formulas.md)
