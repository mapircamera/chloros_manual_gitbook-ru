...
После импорта изображений, маркировки калибровочных целей и настройки параметров проекта вы готовы приступить к обработке. На этой странице приведены инструкции по запуску конвейера обработки Chloros.

## Контрольный список перед обработкой

Перед нажатием кнопки «Пуск» убедитесь, что все готово:
...
* Значок: **кнопка «Воспроизведение/Пуск»** <img src="../.gitbook/assets/image (2) (1).png" alt="" data-size="line">
* Статус: кнопка активирована (светится), когда программа готова к обработке
...
1.**Обнаружение цели** — сканирование в поисках целей калибровки.
...
* Дополнительное ускорение GPU (CUDA) с графическими картами NVIDIA
...
4. **Экспорт** — сохранение обработанных изображений и индексов**Взаимодействие с индикатором выполнения:*** **Наведите курсор мыши** на индикатор, чтобы увидеть подробную раскрываемую панель с 4 этапами
...
* Ускорение GPU дополнительно повышает скорость
...
* Сканирует отмеченные целевые изображения (или все изображения, если ни одно не отмечено)
* Идентифицирует 4 калибровочные панели в каждой цели
* Извлекает значения отражения из целевых панелей
* Записывает временные метки целей для планирования калибровки
...
**Что делает Chloros:*** **Коррекция виньетирования**: устраняет затемнение объектива по краям
...
* Использует соответствующую калибровочную мишень для каждого изображения на основе временной метки
...
* **Смотреть индикатор прогресса** — просматривать общий процент выполнения
...
* **Просматривать готовые изображения** — некоторые экспортные файлы могут появляться во время обработки
...
* Для максимальной чистоты полностью закройте Chloros и перезапустите
...
4. Контролируйте температуру GPU (обеспечьте достаточное охлаждение).
